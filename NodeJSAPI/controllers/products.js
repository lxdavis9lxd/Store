...
exports.getOne = async (req, res, next) => {
	try {
		...
		const data = await model.findOne(productCode);
		if (!_.isEmpty(data)) {
...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(productCode, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
			res.status(StatusCodes.BAD_REQUEST).send({message : "Bad request."});
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(productCode);
		if (data) {
...
exports.getByProductline = async (req, res, next) => {
	try {
		const pageNo = await getPageNo(req);
		const pageSize = await getPageSize(req);
		const offset = (pageNo - 1) * pageSize;
		const productLine = req.params.productLine;
		const totalCount = await model.getByProductlineCount(productLine);
		const data = await model.getByProductline(offset, pageSize, productLine);
		if (!_.isEmpty(data)) {
			const result = {
				pageNo: pageNo,
				pageSize: pageSize,
