...
const checkNumber = req.params.checkNumber;
		...
		const data = await model.findOne(customerNumber,checkNumber);
		if (!_.isEmpty(data)) {
...
const checkNumber = req.params.checkNumber;
		...
		const data = await model.update(customerNumber,checkNumber, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
			res.status(StatusCodes.BAD_REQUEST).send({message : "Bad request."});
...
const checkNumber = req.params.checkNumber;
		...
		//const id = req.params.id;
		const data = await model.remove(customerNumber,checkNumber);
		if (data) {
