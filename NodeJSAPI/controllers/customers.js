...
exports.update = async (req, res, next) => {
	try {
		...
		const data = await model.update(customerNumber, req.body);
		if (!_.isEmpty(data)) {
			res.status(StatusCodes.OK).send(data[0]);
		} else {
			res.status(StatusCodes.BAD_REQUEST).send({message : "Bad request."});
...
exports.remove = async (req, res, next) => {
	try {
		...
		//const id = req.params.id;
		const data = await model.remove(customerNumber);
		if (data) {
...
exports.getBySalesrepemployeenumber = async (req, res, next) => {
	try {
		const pageNo = await getPageNo(req);
		const pageSize = await getPageSize(req);
		const offset = (pageNo - 1) * pageSize;
		const salesRepEmployeeNumber = req.params.salesRepEmployeeNumber;
		const totalCount = await model.getBySalesrepemployeenumberCount(salesRepEmployeeNumber);
		const data = await model.getBySalesrepemployeenumber(offset, pageSize, salesRepEmployeeNumber);
		if (!_.isEmpty(data)) {
			const result = {
				pageNo: pageNo,
				pageSize: pageSize,
