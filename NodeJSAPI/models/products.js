...
exports.findOne = async (productCode) => {
    const query = `SELECT  pu.textDescription as productLine_Value, t.* FROM products t  join productlines pu on t.productLine = pu.productLine  WHERE t.productCode=? LIMIT 0,1`;
...
exports.count = async () => {
    const query = `SELECT count(*) TotalCount FROM products t  join productlines pu on t.productLine = pu.productLine  `;
...
exports.getByProductline = async (offset, pageSize, productLine) => {
    const query = `SELECT  pu.textDescription as productLine_Value, t.* FROM products t  join productlines pu on t.productLine = pu.productLine  WHERE t.productLine= ? LIMIT ?, ?`;
    return getRows(query,[productLine,offset,pageSize]);
}

exports.getByProductlineCount = async (productLine) => {
    const query = `SELECT count(*) TotalCount FROM products t  join productlines pu on t.productLine = pu.productLine  WHERE t.productLine= ?`;
